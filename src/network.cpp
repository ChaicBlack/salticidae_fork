...
void msgnetwork_config_burst_size(msgnetwork_config_t *self,
                                  size_t burst_size) {
...
void msgnetwork_reg_handler(msgnetwork_t *self, _opcode_t opcode,
  ...
  self->set_handler(
      opcode, [=](const msgnetwork_t::Msg &msg, const msgnetwork_conn_t &conn) {
...
void msgnetwork_reg_error_handler(msgnetwork_t *self,
                                  ...
                                  void *userdata) {
  self->reg_error_handler(
      [=](const std::exception_ptr _err, bool fatal, int32_t async_id) {
        SalticidaeCError cerror;
        try {
...
peernetwork_conn_t *peernetwork_conn_new_from_msgnetwork_conn_unsafe(
    const msgnetwork_conn_t *conn) {
  return new peernetwork_conn_t(
      salticidae::static_pointer_cast<peernetwork_t::Conn>(*conn));
}
...
void peernetwork_reg_unknown_peer_handler(
    ...
    void *userdata) {
  self->reg_unknown_peer_handler(
      [=](const NetAddr &claimed_addr, const salticidae::X509 *cert) {
...
clientnetwork_conn_t *clientnetwork_conn_new_from_msgnetwork_conn_unsafe(
    const msgnetwork_conn_t *conn) {
  return new clientnetwork_conn_t(
      salticidae::static_pointer_cast<clientnetwork_t::Conn>(*conn));
}

clientnetwork_conn_t *
clientnetwork_conn_copy(const clientnetwork_conn_t *self) {
