...
public:
  ...
  void release_ref(Node *u) {
    if (u->refcnt.fetch_sub(1, std::memory_order_relaxed) != 1)
      return;
    for (;;) {
  ...
  bool pop(Node *&r) {
    ...
    while (loop) {
      ...
      if (u->refcnt.compare_exchange_weak(t, t + 1,
...
protected:
  ...
  template <typename U> bool _enqueue(U &&e, bool unbounded = true) {
    for (;;) {
      auto t = tail.load(std::memory_order_acquire);
      auto tcnt = t->refcnt.load(std::memory_order_relaxed);
      if (!tcnt)
        continue;
      if (!t->refcnt.compare_exchange_weak(tcnt, tcnt + 1,
                                           std::memory_order_relaxed))
        continue;
      ...
      if (tt >= MPMCQ_SIZE) {
        if (t->next.load(std::memory_order_relaxed) == nullptr) {
          ...
          Block *tnext = nullptr;
          if (!t->next.compare_exchange_weak(tnext, nblk,
      ...
      auto tt2 = tt;
      if (t->tail.compare_exchange_weak(tt2, tt2 + 1,
                                        std::memory_order_relaxed)) {
...
public:
  ...
  ~MPMCQueue() {
    for (FreeList::Node *ptr; blks.pop(ptr);)
      delete ptr;
  ...
  template <typename U> bool try_enqueue(U &&e) { return _enqueue(e, false); }
  ...
  bool try_dequeue(T &e) {
    for (;;) {
      ...
      if (!hcnt)
        continue;
      if (!h->refcnt.compare_exchange_weak(hcnt, hcnt + 1,
                                           std::memory_order_relaxed))
        continue;
      ...
      if (hh >= tt) {
        ...
        auto h2 = h;
        if (this->head.compare_exchange_weak(h2, hnext,
                                             std::memory_order_acq_rel))
          this->blks.push(h);
        blks.release_ref(h);
        continue;
      ...
      auto hh2 = hh;
      if (h->head.compare_exchange_weak(hh2, hh2 + 1,
                                        std::memory_order_relaxed)) {
...
template <typename T> struct MPSCQueue : public MPMCQueue<T> {
  ...
  template <typename U> bool rewind(U &&e) {
    auto h = this->head.load(std::memory_order_relaxed);
    auto hh = h->head.load(std::memory_order_relaxed);
    if (!hh) {
      FreeList::Node *_nblk;
      if (!this->blks.pop(_nblk))
        _nblk = new typename MPMCQueue<T>::Block();
